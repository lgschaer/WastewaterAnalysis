...
richness <- bbps %>%
  ...
  separate(SampleID, into = c("X", "SampleID"), sep = "X")%>%
  as_tibble()
richness$X <- NULL
head(richness)
...
rich_summary <- richness %>%
  left_join(sdata2, by = "SampleID") %>%                                         #join to meta data
  #filter(datatype=="swab"|datatype=="bilge"|datatype=="water"|datatype=="dock"|datatype=="air") %>%    #filter to keep only sample types we want
...
#-------------------------------------------------#
#DETERMINE IF THERE ARE SULFUR DEGRADING ORGANISMS
...
#make table of wastewater organisms present in data
wworg <-sulf2 %>%
...
#make color palette
colors1 <- c(
  "purple4", "darkcyan",     "orchid1",   "green",       "coral1",        "blue",
  "grey47",  "cyan",         "yellow",    "darkgreen",   "palegoldenrod",     "tan4",
  "grey77",  "darkblue",     "orange",    "red",         "mediumpurple1",
  "white",   "dodgerblue",   "firebrick", "yellowgreen", "magenta", "black"
)  
...
ggplot(sulf2)+
  ...
  theme_minimal()+
  theme(axis.text.y.left = element_text(size = 20),
        axis.text.x = element_text(size = 20, angle = 0, vjust = 0.5, hjust = 1),
        axis.title.y = element_text(size = 20),
